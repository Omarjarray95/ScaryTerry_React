...
export function getEvents()
{
    ...
    const request = axios.get('http://localhost:3001/meetings/calendar', {
        params: {
...
export function addEvent(newEvent)
{
    return (dispatch, getState) => {
        newEvent.desc=localStorage.getItem('curentSprint')
        const request = axios.post('http://localhost:3001/meetings/restadd', {
            newEvent
        });
...
{   event.desc=localStorage.getItem('id');
    return (dispatch, getState) => {
            event.name="test";
        const request = axios.post('http://localhost:3001/meetings/restedit', {
            event
        });
